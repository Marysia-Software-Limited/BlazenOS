...
9. **Python + Rust only.** The two implementation languages are fixed.
   New components pick one per the rules in
...
- **Source-of-truth docs** live under `docs/`. Anything that contradicts
...
7. **Right rig for the job.** `paul` (Linux) is the **primary** rig for
   `blazen_os`; image builds, cross-compile, and full Tier 2-3 tests
   run there. macOS handles `make dev` smokes only — its primary repo
   is `rachel` (the mobile twin). See
   [`docs/15-DEV-WORKFLOW.md`](docs/15-DEV-WORKFLOW.md). `make sync-paul`,
   `make paul-test-fast`, `make paul-vm-image` are the wrapper targets.
   Cross-repo coordination on `docs/product/` (the shared spec) goes
   through git or rsync between the two Claude sessions.
