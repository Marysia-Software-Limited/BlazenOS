...
3. Lokalny LLM (domyślnie **Bielik 4.5B** — polski model SpeakLeash — na
   llama.cpp; bogatszy polski, ~2 tok/s na CPU 8 GB, snappy fallback to
   **Bielik 1.5B**; opcjonalnie wariant `.hef` na akceleratorze Hailo) decyduje
...
| LLM (CPU) | [llama.cpp](https://github.com/ggml-org/llama.cpp) | **Bielik 4.5B** Q6_K default (Polish-first, SpeakLeash; built on Qwen2.5 so EN is retained) — richer Polish, ~2 tok/s on 8 GB CPU; **Bielik 1.5B** = snappy fallback (9.6 tok/s). See [docs/05-MODELS.md](docs/05-MODELS.md). |
