#!/usr/bin/env python3
"""HAT voice-recognition check — record from the ReSpeaker 2-Mics (WM8960)
mic and transcribe with faster-whisper.

This is a *hardware* diagnostic: it proves the real capture path on the
ReSpeaker HAT plus on-device, multilingual (Polish-first) speech recognition
end to end, independent of the appliance's own (still-stubbed) `blazend-asr`
unit. Models run on the Pi 5 CPU (int8); nothing leaves the device.

Usage (speak when it says "MÓW TERAZ / SPEAK NOW"):

    .venv/bin/python scripts/hat-voice-check.py                 # 5 s, pl, small
    .venv/bin/python scripts/hat-voice-check.py --lang auto     # let it detect
    .venv/bin/python scripts/hat-voice-check.py --seconds 7 --model medium
    .venv/bin/python scripts/hat-voice-check.py --loop          # keep listening

The HAT must be enabled (card `wm8960soundcard`). See HANDOFF.md
2026-06-15 session log for the one-time `config.txt` overlay setup.
"""
...
DEFAULT_DEVICE = "plughw:CARD=wm8960soundcard,DEV=0"
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="hat-voice-check")
