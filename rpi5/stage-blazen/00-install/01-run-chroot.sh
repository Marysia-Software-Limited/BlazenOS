...
# Local pure-python domain wheels (staged unconditionally by build-image.sh):
# jessica-linux provides the `jessica` console script that
# blazend-fabric-snapshot / blazend-pull-catalog ExecStart — without it those
# units die with 203/EXEC. Only external dep in this closure is pyyaml
# (installed above), so --no-index resolves. NO `|| true`: staged-but-broken
# wheels must fail the build loudly.
if [ -d "$STAGE/blazen-local-wheels" ]; then
  echo "=== blazend chroot: installing local domain wheels ==="
  "$INSTALL_DIR/.venv/bin/pip" install --no-cache-dir --no-index \
    --find-links "$STAGE/blazen-local-wheels" \
    jessica-linux audiobook-catalog mesh-registry mesh-llm context-sync
fi
...
mkdir -p /usr/share/blazen/defaults /etc/blazen/intents /etc/blazen/overrides \
         /etc/blazen/ontology /etc/blazen/prompts
...
# Site layer (/etc/blazen) — mirrors the live appliance: several units
# hard-reference /etc/blazen/<name>.yaml, and the ConfigLoader gives this
# layer precedence over /usr/share/blazen/defaults.
for f in asr audio audiobooks embeddings fabric llm news phrases radio wake-word weather; do
  cp "$STAGE/blazen-configs/$f.yaml" "/etc/blazen/$f.yaml"
done
# Site-DIVERGENT mesh registry: paul's llm resource is stripped at stage time
# (Decision 2026-08-09 — the Pi never routes to paul's Ollama). Generated by
# build-image.sh into blazen-configs/site/.
cp "$STAGE/blazen-configs/site/mesh.yaml" /etc/blazen/mesh.yaml
cp "$STAGE/blazen-configs/ontology/"*.json /etc/blazen/ontology/
cp "$STAGE/blazen-configs/prompts/"*.json  /etc/blazen/prompts/
# secrets.env: 0600 comment-only template — REAL KEYS NEVER SHIP IN AN IMAGE.
# systemd reads EnvironmentFile= as root, so root:root 0600 is correct.
if [ ! -f /etc/blazen/secrets.env ]; then
  printf '# Cloud opt-in keys (OPENAI_API_KEY=..., GEMINI_API_KEY=...).\n# Filled by the operator post-boot. Never committed, never baked into images.\n' \
    > /etc/blazen/secrets.env
fi
chmod 0600 /etc/blazen/secrets.env
...
# blazend-wake IS enabled: a REAL "dżesika" wake model ships
# (models/wake/jessica.onnx — retrained 2026-08-09 on 853 screened real
# negatives + the operator's own utterances). The ASR capture is wake-triggered
# — the ASR subscribes to `wake.detected` and reads a fixed window around each
# hit — so the wake detector is a hard dependency of the voice path, not an
# optional gate. Threshold/cooldown live in blazend-wake.service (0.8 /
# 3000 ms). See docs/05-MODELS.md and configs/wake-word.yaml.
#
# blazend-button (ReSpeaker HAT GPIO17) is deliberately NOT enabled — this is
# the Jabra-only appliance. The unit file still ships; HAT users can
# `systemctl enable blazend-button` by hand.
for unit in blazend-audio-in blazend-audio-out blazend-wake blazend-asr \
            ...
            blazend-orchestrator blazend-bootstrap blazend-fabric \
            blazend-fabric-snapshot; do
  systemctl enable "${unit}.service" 2>/dev/null || true
done
# The audiobook catalog refresher is timer-driven (timers need their own enable).
systemctl enable blazend-pull-catalog.timer 2>/dev/null || true
...
# Full persistent-state tree (mirrors usr/lib/tmpfiles.d/blazen.conf, which
# self-heals these on every boot — this chroot mkdir just guarantees the model
# bake above and any post-flash rsync land in a blazen-owned tree from boot #1).
mkdir -p /var/lib/blazen/{data,voice-cache,wake-negatives,music,semantic-index,audiobooks,fabric,media,models} /run/blazen
