...
The orchestrator's :class:`blazend.led.LedSimulator` derives the active status
...
Colour vocabulary is imported from :mod:`blazend.led` so there is exactly one
...
from blazend.led import BLUE, GREEN, MAGENTA, OFF, RED, YELLOW

log = logging.getLogger("blazend.led_hw")

# Contract colour → 8-bit (R, G, B). Keep the keys in sync with blazend.led.
...
def _diag(hold_s: float = 0.6) -> None:
    ...
    ``python -m blazend.led_hw`` — the headless way to confirm the LEDs (and the
